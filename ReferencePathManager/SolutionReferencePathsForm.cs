  ...
  public partial class SolutionReferencePathsForm : Form
  {
    ...
    protected override void OnLoad( EventArgs e )
    {
      ...
      foreach ( Project proj in GetAllProjects( applicationObject.Solution ) )
      {
        ...
        if ( props == null ||
             proj.Kind != "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}" /* PrjKind.prjKindCSharpProject */ &&
             proj.Kind != "{F184B08F-C81C-45F6-A57F-5ABD9991F28F}" /* PrjKind.prjKindVBProject     */ )
        {
          continue;
        }
      ...
      KeyPreview = true;
      KeyDown += SolutionReferencePathsForm_KeyDown;
    }

    private void SolutionReferencePathsForm_KeyDown( object sender, KeyEventArgs e )
    {
      if ( e.KeyCode == Keys.V && e.Modifiers == Keys.Control ) HandleAddFromClipboard();
    }
    ...
    private void HandleAddFromClipboard( object sender, EventArgs e )
    {
    ...
    {
      foreach ( var path in pathList ) DeletePath( projects, path );
    }
    ...
    {
      foreach ( var path in pathList ) AppendPath( projects, path );
    }
